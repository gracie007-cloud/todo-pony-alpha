...
import { EmojiPicker, EmojiPickerCompact } from "./EmojiPicker";
...
export function ListForm({
  ...
  const [color, setColor] = React.useState(initialData?.color || "#6366f1");
  const [emoji, setEmoji] = React.useState<string | null>(initialData?.emoji || null);
    ...
    <form onSubmit={handleSubmit} className={cn("space-y-4", className)}>
      ...
      <div className="flex items-center justify-center p-4 rounded-lg bg-muted/50">
        ...
        >
          ...
          >
            {emoji || name.charAt(0).toUpperCase() || "L"}
          </div>
          <span
...
// Compact inline form for quick list creation
export function ListFormInline({
  onSubmit,
  className,
}: {
  onSubmit: (data: ListFormData) => void;
