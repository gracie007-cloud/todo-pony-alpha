...
import { useTasksByList, useTaskMutations, useList } from "@/lib/hooks";
...
export default function ListPage() {
  ...
  const { tasks, isLoading, isError, mutate } = useTasksByList(listId);
  ...
  if (isError || !list) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <p className="text-destructive">Failed to load list</p>
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          >
            ...
            >
              {list.emoji ? (
                ...
                <Folder className="size-4" style={{ color: list.color }} />
              )}
            </div>
          ...
          >
            {incompleteTasks.length} task{incompleteTasks.length !== 1 ? "s" : ""} in this list
          </motion.p>
        </div>
        <motion.div
